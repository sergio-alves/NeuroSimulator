 ...
 */
class Neuron(_n: NerveCell) extends JComponent with DragGestureListener with DragSourceListener {
  def this(nct: NerveCellType, uuid: String, _name: String, _threshold: Double, description: String) = {
    this(NerveCellFactory.createNerveCell(nct, uuid, _name, _threshold, description))    
  }
